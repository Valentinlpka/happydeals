...
exports.createPayment = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (isWeb) {
      ...
      await admin.firestore().collection("pendingPayments").add({
        paymentIntentId: paymentIntent.id,
        customer: customer.id,
        amount,
        currency,
        sellerId: connectAccountId,
        status: "pending",
...
// Nouvelle fonction pour capturer le paiement et transférer les fonds
exports.capturePendingPayment = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      const pendingPaymentDoc = await admin
        .firestore()
      ...
      const pendingPayment = pendingPaymentDoc.docs[0].data();
      ...
      await pendingPaymentDoc.docs[0].ref.update({
        status: "captured",
...
// Nouvelle fonction pour que les vendeurs reçoivent leurs fonds
exports.receiveFunds = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "User must be authenticated."
  ...
  try {
    ...
    const pendingPayments = await admin
      .firestore()
      .collection("pendingPayments")
      .where("sellerId", "==", stripeAccountId)
    ...
    if (totalAmount > 0) {
      ...
      await stripe.transfers.create({
        amount: totalAmount,
        currency: "eur", // Assurez-vous que c'est la bonne devise
        destination: stripeAccountId,
      });

      // Mettre à jour les documents de paiement en attente
      await batch.commit();
    }

    return { success: true, amountTransferred: totalAmount };
  } catch (error) {
